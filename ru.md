...
- **фото документа** (камера или галерея);
- **текст расходов** (вставка, веб-страница по URL, QR/штрихкод);
...
Содержимое документа (изображение или текст) **не сохраняется в нашей базе данных**. Мы **не ведём журнал** распознавания. Обработка на стороне OpenAI регулируется [их политикой конфиденциальности](https://openai.com/policies/privacy-policy).
...
- мы получаем информацию о статусе подписки/покупки через **RevenueCat** (идентификатор пользователя, продукт, срок действия, баланс AI Tokens).
...
- распознавание расходов по вашему запросу;
- формирование отчётов с пересчётом сумм в выбранную валюту (курсы валют);
- предоставление платных функций (подписка, AI Tokens);
- экспорт данных;
...
| **OpenAI** | распознавание расходов (по вашему действию) | [openai.com/policies/privacy-policy](https://openai.com/policies/privacy-policy) |
